...
class ProtExtracRandomtCoordSubparticles(ProtParticlePicking, ProtParticles):
    ...
    """
    _label = 'extract random subparticles coordinates'
    ...
    def createOutputStep(self):
        ...
        coordsDict = {}
        for coord in outputSet:
            particleKey = str(coord.getMicId())
        ...
        for key in coordsDict:
            setcords = coordsDict[key]
            idxArray = np.arange(0, len(setcords))
            ...
            idxArrayPermutations = idxArrayPermutations[0:random_number]

            newsetcoords = np.array(setcords)[idxArrayPermutations.astype(int)]

            for coor in newsetcoords:
                myCoor = Coordinate()
                myCoor.setMicName(None)
                myCoor.setMicId(key)
                myCoor.setObjId(idx)
                myCoor.setPosition(coor[0], coor[1])
                outputSet.append(myCoor)
                idx += 1
